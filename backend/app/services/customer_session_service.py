...
class CustomerSessionService:
    ...
    def get_active_customer_session_for_table(
        ...
        if table is None:
            raise TableNotFoundError(
                f"Table {table_number} does not exist"
            )

        customer_session = (
            self.customer_session_repo.get_active_session_for_table(
                table.id
            )
        )

        if customer_session is None:
            raise CustomerSessionNotFoundError(
                f"No ACTIVE customer session exists for table {table_number}"
            )
    ...
    def create_customer_session(
        ...
        *,
        ...
        customer_session = (
            self.customer_session_repo.create_customer_session(
                table_id=table.id,
                name=name,
                email=email,
    ...
    def get_customer_session(
        ...
        customer_session = (
            self.customer_session_repo.get_customer_session_by_id(
                session_id
            )
        )
        ...
        if customer_session is None:
            raise CustomerSessionNotFoundError(
                f"Customer session {session_id} was not found"
            )
    ...
    def update_customer_session(
        ...
        *,
        ...
        if table_number is not None:
            table = self.customer_session_repo.get_table_by_number(
                table_number
            )
            ...
            if (
                existing_active_session is not None
                and existing_active_session.id != customer_session.id
            ):
                raise TableSessionConflictError(
                    f"Table {table_number} already has an ACTIVE customer session"
                )
        ...
        updated_customer_session = (
            self.customer_session_repo.update_customer_session(
                customer_session,
                **update_payload,
            )
        )
